...
All outbound HTTP requests use **HTTPS/TLS** to AWS-owned CloudFront distributions:
- `d1qsjq9pzbk1k6.cloudfront.net` — service manifests and definitions (read-only)
- `dnd5zrqcec4or.cloudfront.net` — estimate save API (write)
...
|---|---|---|
| Payload injection via descriptions | Medium | `sanitize()` strips HTML/XML characters before API submission |
| Unbounded in-memory storage | Low | Local process; estimates are ephemeral. Acceptable for sample code. |
| Env var URL override redirecting traffic | Low | User-controlled env vars on their own machine. No secrets transmitted. |
| Dependency supply chain | Low | Single runtime dependency (`@modelcontextprotocol/sdk`). `package-lock.json` committed for reproducible builds. |
