...
class EstimateBuilder {
  ...
  addService(compositeKey, config, { group } = {}) {
    let candidate = compositeKey;
    if (this.usedKeys.has(candidate) && config?.description) {
      candidate = `${compositeKey}:${config.description.replace(/\s+/g, '')}`;
    }
    let suffix = 2;
    while (this.usedKeys.has(candidate)) {
      candidate = `${compositeKey}:${suffix}`;
      suffix++;
    }
        ...
        if (svc.subType === 'subService' && subServiceParent.has(svcKey)) {
          ...
          out[`${parent.key}-${crypto.randomUUID()}`] = {
            ...
            subServices: [{
              ...
              region,
              estimateFor: this._inferTemplateId(subDef, config),
