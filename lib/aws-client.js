...
function exampleForField(field) {
  if (field.type === 'fileSize') return { value: '100', unit: field.defaultUnit || 'gb|NA' };
  if (field.type === 'frequency') return { value: '1', unit: field.unit || 'perMonth' };
  if (field.type === 'durationInput') return { value: '1', unit: field.unit || 'hours' };
  if (field.type === 'percentInput') return '100';
  if (field.type === 'numericInput') return '1';
  if (field.type === 'dropdown' && field.options?.length > 0) return field.options[0].id;
  if (field.type === 'columnFormIPM') {
    const row = {};
    for (const item of field.row || []) {
      const key = item.selectorId || item.label || 'undefined';
      if (item.type === 'utilization') row[key] = { value: { unit: '100', selectedId: '%Utilized/Month' } };
      else row[key] = { value: item.isInstanceType ? '<instance-type>' : '1' };
    }
    return { value: [row] };
  }
  return '1';
...
function valueShapeForField(field) {
  if (field.type === 'fileSize') return 'object: {"value":"<number>","unit":"<size>|<frequency>"}';
  if (field.type === 'frequency') return 'object: {"value":"<number>","unit":"<frequency-unit>"}';
  if (field.type === 'durationInput') return 'object: {"value":"<number>","unit":"<duration-unit>"}';
  if (field.type === 'dropdown') return 'string: one option id';
  if (field.type === 'columnFormIPM') return field.valueShape;
  return 'string or number';
}

function fieldToConfigSchema(field) {
  const out = {
    id: field.id,
    label: field.label || field.id,
    type: field.type,
    valueShape: valueShapeForField(field),
    example: exampleForField(field),
  };
  if (field.templateId) out.templateId = field.templateId;
  if (field.unit) out.unit = field.unit;
  if (field.defaultUnit) out.defaultUnit = field.defaultUnit;
  if (field.validSizes) out.validSizes = field.validSizes;
  if (field.options) out.options = field.options;
  if (field.row) out.row = field.row;
  if (field.selectorValues) out.selectorValues = field.selectorValues;
  return out;
}

function buildServiceConfigSchema(service, fields) {
  return {
    ...
    configShape: {
      ...
      fields: 'add calculator fields as keys using the field IDs below',
    },
    recommendedFlow: [
      'Confirm region with the user before adding services.',
      'Use field IDs exactly as shown.',
      'For duplicate/versioned fields, prefer the highest version suffix.',
      'Batch multiple line items in one add_services_structured call.',
    ],
    fields: fields.map(fieldToConfigSchema),
  };
}
...
module.exports = { PARTITIONS, resolvePartition, loadManifest, findService, searchServices, fetchServiceDefinition, saveEstimate, extractInputFields, enrichFieldsWithMetadata, buildServiceConfigSchema, parseDoubleEncodedResponse, fetchEstimate, estimateToMarkdown };
